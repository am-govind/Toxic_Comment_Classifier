...
class TestPredictResponseStructure:
    ...
    def test_result_has_required_fields(self, client):
        ...
        required_fields = ["text", "scores", "is_toxic", "severity", "flagged_categories"]
...
class TestClassificationLogic:
    ...
    def test_toxic_comment_is_flagged(self, client):
        ...
        data = client.post("/predict", json={
            "comments": ["You are the worst, most stupid, disgusting idiot I've ever seen"],
            "threshold": 0.1,
        }).json()
        result = data["results"][0]
    ...
    def test_threshold_affects_classification(self, client):
        ...
        low_result = client.post("/predict", json={
            "comments": comment,
            "threshold": 0.05,
        }).json()["results"][0]

        high_result = client.post("/predict", json={
            "comments": comment,
            "threshold": 0.95,
        }).json()["results"][0]
